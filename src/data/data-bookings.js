...
function generateRandomData(count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        //Unconfirmed
        if (i < 15) {
            start = generateRandomNumber(1, 30);
            end = start + generateRandomNumber(5, 15);
        }
        // Checked-In
        else if (i >= 15 && i < 30) {
            start = generateRandomNumber(-15, 0);
            end = start + generateRandomNumber(5, 15);
        }
        // Checked-Out
        else {
            start = generateRandomNumber(-45, -1);
            end = start + generateRandomNumber(5, 15);
        }

        const created_at = fromToday(-20, true);
        const startDate = fromToday(start);
        const endDate = fromToday(end);
        const cabinId = cabins[Math.floor(Math.random() * cabins.length)];
        const guestId = i + 1;
        const hasBreakfast = Math.random() > 0.5;
        const observations = generateRandomObservations();
        const isPaid = Math.random() > 0.5;
        const numGuests = Math.floor(Math.random() * 11);
...
function generateRandomObservations() {
    const observations = [
        "I have a gluten allergy and would like to request a gluten-free breakfast.",
        "Please provide a crib for my baby.",
        "I would like a late checkout.",
        "I have a request for a specific view from my cabin.",
        "Please ensure the room is cleaned daily.",
        "I have a pet and would like to inquire about your pet policy.",
        "I have a special request for my anniversary celebration.",
        "Please provide extra towels and toiletries.",
        "I have a question about your cancellation policy.",
        "I would like to book additional activities or services.",
    ];

    return observations[Math.floor(Math.random() * observations.length)];
}

export const bookings = generateRandomData(50);
